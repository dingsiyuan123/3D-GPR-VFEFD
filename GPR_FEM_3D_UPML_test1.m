...
%                   Using UPML and Hexahedral mesh                                  %
...
R = exp(-16);
eta0 = 120*pi;
sigmax = -(m+1) * log(R)/(2*npml*dx)/eta0;

% x direction
xdel = zeros(Ny,Nx,Nz);
for i=1:npml
    xdel(:,i,:)=(npml+1-i)/npml;
end
xdel(:,Nx-npml+1:Nx,:)=xdel(:,npml:-1:1,:);

% y direction
ydel = zeros(Ny,Nx,Nz);
for i=1:npml
    ydel(i,:,:)=(npml+1-i)/npml;
end
ydel(Ny-npml+1:Ny,:,:)=ydel(npml:-1:1,:,:);

% z direction
zdel = zeros(Ny,Nx,Nz);
for i=1:npml
    zdel(:,:,i)=(npml+1-i)/npml;
end
zdel(:,:,Nz-npml+1:Nz)=zdel(:,:,npml:-1:1);

sigx = sigmax.*xdel.^m;
sigy = sigmax.*ydel.^m;
sigz = sigmax.*zdel.^m;

Sx =  1-(sigx*1i/omega/ep0);
Sy =  1-(sigy*1i/omega/ep0);
Sz =  1-(sigz*1i/omega/ep0);
